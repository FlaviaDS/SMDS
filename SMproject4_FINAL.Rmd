...
plot(d.train$Hos_symp, d.train$Variation_cases, col=4, lwd=2, xlab="Hospitalised with symptoms", ylab="Variation_cases" ) #Variation_cases has Hos_symp inside


plot(d.train$Variation_pos, d.train$Intensive_care, col=4, lwd=2, xlab="Variation_pos", ylab="Intensive_care")

```
``` {r}
#relationship between response variable and covariates

hist(d.train$Variation_pos, freq = T, breaks = 30, xlab = "Variation in positives", ylab="", xlim = c(-17000,4000))
...
plot(d.train$log_Recovered_today, d.train$Intensive_care , col=2, lwd=2)
...
plot(model.glm.quasi$fitted.values,z.glm.quasi, xlab = "Fitted values", ylab = "std. residuals", main = "Standardized residuals- Quasipoisson model", ylim=c(-3,3))
abline(h=-1.96, col="red")
abline(h=1.96, col="red")
...
plot(d.train$Test_today, d.train$Intensive_care, xlab = "Test_today", ylab="Fitted values")
points(d.train$Test_today, model.glm$fitted.values,col="blue", pch=19,bg="blue")
points(d.train$Test_today, model.glm.nb$fitted.values, col="green", pch=20, bg="green")
legend("bottomright", c("GLM","NB"), pch=c(19,20), col=c("blue", "green"))
...
plot(d.train$Test_today, d.train$Intensive_care, xlab = "Test_today", ylab="Fitted values")
points(d.train$Test_today, model.glm$fitted.values,col="blue", pch=19,bg="blue")
points(d.train$Test_today, model.gam$fitted.values, col="red", pch=20, bg="red")
legend("bottomright", c("GLM","GAM"), pch=c(19,20), col=c("blue", "red"))

```


It seems that negative binomial is the best model.

```{r}
#random forest
library(randomForest)
model.rf <- randomForest(Intensive_care ~ Date+Variation_cases+Death_today+Test_today,
...
```
``` {r}
# #commenting the estimates of nb
# 
# exp(model.nb$coefficients %*% c(1,1, mean(d.train$Variation_cases)+100, (mean(d.train$Variation_cases)+100)^2, mean(d.train$Death_today)+(mean(d.train$Death_today))^2+ (mean(d.train$Death_today))^3+ mean(d.train$Test_today))- 
...
library(ggplot2)
 ggplot()+ 
   ...
   scale_color_manual(name= "models", values = c("actual"="green","Poisson"="red", "Quasi Poisson"= "blue", "Negative Binomial"= "orange", "GAM"= "pink" , "RF"="purple"))+
...
plot(d.train$Date, d.train$Season,  col=2, lwd=2)
...
```{r}
#adding Zone_color
model.glm1 <- glm(Intensive_care ~ Date+poly(Variation_cases,2)+Test_today+
                   poly(Death_today,3)+Zone_color, family = poisson, data=d.train)
summary(model.glm1)
aic <- AIC(model.glm1)
bic <- BIC(model.glm1)
...
model.glm1 <- glm(Intensive_care ~ Date+poly(Variation_cases,2)+Test_today+
                   poly(Death_today,3)+Zone_color+ lag_zone_color, family = poisson, data=d.train)
summary(model.glm1)
aic <- AIC(model.glm1)
bic <- BIC(model.glm1)
RD <- model.glm1$deviance
...
#adding season
model.glm1 <- glm(Intensive_care ~ Date+poly(Variation_cases,2)+Test_today+
                   poly(Death_today,3)+Zone_color+ lag_zone_color+Season, family = poisson, data=d.train)
summary(model.glm1)
aic <- AIC(model.glm1)
bic <- BIC(model.glm1)
RD <- model.glm1$deviance
...
#adding interaction
model.glm1 <- glm(Intensive_care ~ Date+poly(Variation_cases,2)+Test_today+
                   poly(Death_today,3)+Zone_color+lag_zone_color+Season,
                  family = poisson, data=d.train)
summary(model.glm1)
aic <- AIC(model.glm1)
bic <- BIC(model.glm1)
...
```{r}
# new quasi piosson
model.glm.quasi1 <- glm(Intensive_care ~ Date+poly(Variation_cases,2)+ 
                   poly(Death_today,3)+Test_today+Zone_color+lag_zone_color+Season,
                   family = quasipoisson, data=d.train)
...
# nb
library(MASS)
model.glm.nb1 <- glm.nb(Intensive_care ~ Date+poly(Variation_cases,2)+ 
                   poly(Death_today,3)+Test_today+ Zone_color+lag_zone_color+Season, 
                   data=d.train)
...
plot(d.train$Test_today, d.train$Intensive_care, xlab = "Test_today", ylab="Fitted values")
points(d.train$Test_today, model.glm1$fitted.values,col="blue", pch=19,bg="blue")
points(d.train$Test_today, model.glm.nb1$fitted.values, col="green", pch=20, bg="green")
legend("bottomright", c("GLM","NB"), pch=c(19,20), col=c("blue", "green"))
...
model.gam1 <- gam(Intensive_care ~ Date+s(Variation_cases)+ 
                  s(Death_today)+Test_today+ Zone_color+lag_zone_color+Season
                   , family = poisson, data=d.train)
summary(model.gam1)
model.gam1$sp
...
plot(d.train$Test_today, d.train$Intensive_care, xlab = "Test_today", ylab="Fitted values")
points(d.train$Test_today, model.glm1$fitted.values,col="blue", pch=19,bg="blue")
points(d.train$Test_today, model.gam1$fitted.values, col="red", pch=20, bg="red")
legend("bottomright", c("GLM","GAM"), pch=c(19,20), col=c("blue", "red"))
...
#random forest
library(randomForest)
model.rf1 <- randomForest(Intensive_care ~ Date+Variation_cases+Death_today+Test_today+
                            Zone_color+lag_zone_color+Season, data=d.train, importance=TRUE)
model.rf1
...
```
again NB is the best model.  AIC is lower than the case without new variables for all models; so our new added variables work well.
